import { Play, Clock, Edit2, History as HistoryIcon, Copy, Check } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto w-full pt-8 pb-16 px-4">
      <header className="mb-12 flex flex-col items-center text-center relative">
        <button 
          onClick={handleCopy}
          className="absolute right-0 top-0 text-slate-400 hover:text-indigo-600 transition-colors p-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wider bg-white rounded shadow-sm border border-slate-100"
          title="Copiar link de sincronização"
        >
          {copied ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}
          {copied ? <span className="text-green-600">Copiado!</span> : 'Copiar Link'}
        </button>
